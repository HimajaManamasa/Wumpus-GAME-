...
public class GUI implements ActionListener
{
    ...
    private JLabel statusgold;
    private JLabel statuswumpus;
    private World w;
    private Agent agent;
    ...
    private void createWindow()
    {
        ...
        buttons.add(status);
        //Status label Gold
        statusgold = new JLabel("", SwingConstants.CENTER);
        statusgold.setPreferredSize(new Dimension(150,25));
        buttons.add(statusgold);
        //Status label Wumpus
        statuswumpus = new JLabel("", SwingConstants.CENTER);
        statuswumpus.setPreferredSize(new Dimension(150,25));
        buttons.add(statuswumpus);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(e.getActionCommand().equals("TRAIN")){
            if (agent == null) {
                ...
                for (int i = 0; i < wumpusAI.SPEEDOFROUNDING; i++) {
                    agent.train();
                    updateGame();
                ...
                startMap(seed);
                System.out.println("Agent is trained");
                agent.printQTable();
            }
        }
    }
    ...
    private void updateGame()
    {
        ...
        status.setText("");
        statusgold.setText("");
        statuswumpus.setText("");
        if(w.blockHasGold()) 
        {
            statusgold.setText("Collect the gold!");
        }
        if (w.isInPit())
        {
        ...
        if(w.foundWumpus())
        {
            statuswumpus.setText("Shoot the Wumpus!");
        }
